...
module.exports = {
     ...
     * @param {{sheet, row, col}} ref - cell reference
     * @param {MODE} mode
     */
     ...
     * @param {number} oldRowNum
     * @param {number} newRowNum
     * @return {string}
     */
    replaceRowNumber: (formula, currSheetName, sheetName, oldRowNum, newRowNum) => {
        ...
        formula = formula.replace(rangeReference, (match, sheetMatch, sheet1, sheet2, absCol1, colName1, absRow1, row1, absCol2, colName2, absRow2, row2) => {
            ...
            row1 = Math.min(row1, row2);
            row2 = Math.max(row1, row2);
            if (row1 <= oldRowNum && oldRowNum <= row2 && diff > 0 && Math.abs(row2 - row1) <= diff
                || diff > 0 && oldRowNum > row2 && newRowNum < row1) return '#REF!';
            if (diff > 0 && row1 <= oldRowNum && oldRowNum <= row2) {
                row2 -= diff;
            } else if (diff < 0) {
                if (row1 >= oldRowNum) {
                    row1 -= diff;
                    row2 -= diff;
                } else if (row1 < oldRowNum && oldRowNum <= row2) {
                    row2 -= diff;
                }
            }
            return (sheetMatch ? sheetMatch : '') + (absCol1 ? '$' : '') + colName1 + (absRow1 ? '$' : '') + row1 + ':'
