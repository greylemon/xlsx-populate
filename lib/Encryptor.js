...
class Encryptor {
    ...
    async encrypt(data, password) {
        ...
        const encryptionInfoBuffer = await this._buildEncryptionInfo(encryptionInfo);
    ...
    decryptAsync(data, password) {
        ...
        let encryptionInfoBuffer = parsed.FileIndex.find(item => item.name === 'EncryptionInfo').content;
        let encryptedPackageBuffer = parsed.FileIndex.find(item => item.name === 'EncryptedPackage').content;
     ...
     * @private
     */
    async _buildEncryptionInfo(encryptionInfo) {
        ...
        const encryptionInfoXml = await xmlBuilder.build(encryptionInfoNode);
