...
module.exports = {
     ...
     * @param {string} name - The name of the child node.
     * @param {{}} attributes - The attributes to set.
     * @param {Array.<string>|undefined} [nodeOrder] - The order of the node names.
     * @returns {{}} The child.
     */
    setChildAttributes(node, name, attributes, nodeOrder) {
        let child = this.findChild(node, name);
        _.forOwn(attributes, (value, attribute) => {
            if (_.isNil(value)) {
                ...
                if (!child) {
                    child = { name, attributes: {}, children: [] };
                    if (nodeOrder) {
                        this.insertInOrder(node, child, nodeOrder);
                    } else {
                        this.appendChild(node, child);
                    }
                }
