...
class Theme {
    ...
    constructor(node) {
        this._node = node;
        this._themeColorNode = node['a:theme']['a:themeElements']['a:clrScheme'];
    ...
    themeColor(index, tint) {
        let color;
        let colorTag = this._themeColorNode[this._colorKeys[index]];
        colorTag = colorTag[Object.keys(colorTag)[0]];
        if (index === 0 || index === 1) {
            color = colorTag[Object.keys(colorTag)[1]];
        } else {
            color = colorTag[Object.keys(colorTag)[0]];
        }
