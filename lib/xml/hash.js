...
function BrowserStream() {
    return {
        ...
        digest: () => {
            return crypto.subtle.digest('SHA-1', encoder.encode(this.buf));
        }
    };
}
...
const hash = async (object) => {
    ...
    hashingStream = isNode ? crypto.createHash(options.algorithm) : BrowserStream();
...
function typeHasher(options, hashingStream, context) {
    context = context || [];
