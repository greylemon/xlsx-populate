...
module.exports = {
     ...
     * @param {ChildChecker} [checker] - The checker function.
     * @returns {undefined|{}} The child if found.
     */
    findChild(node, name, checker) {
        const children = node.children;
        if (!children) return;
        for (let i = 0; i < children.length; i++) {
            const child = children[i];
            if (child.name === name && (checker ? checker(child) : true))
                return child;
        }
    },
     ...
     * @param {string} name - The name to find.
     * @param {ChildChecker} checker - The checker function.
     * @returns {undefined|Array} The child if found.
     */
    findAllChildren(node, name, checker) {
        const children = node.children;
        const result = [];
        if (!children) return;
        for (let i = 0; i < children.length; i++) {
    ...
    removeChild(node, child) {
        ...
        if (typeof child === 'string') {
            ...
            for (let i = 0; i < idxToRemove.length; i++) {
                children.splice(idxToRemove[i], 1);
            }
        } else {
     ...
     * @param {{attributes, name, children}} node - The node.
