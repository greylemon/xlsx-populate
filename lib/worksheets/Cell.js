...
class Cell {
    ...
    setCellPermit(newPermit) {
      this._readOnly = newPermit;
      return this._readOnly;
    }
    ...
    setPrepopulate(setting) {
      this._prepopulate = setting;
      return this._prepopulate;
    }
    ...
    clear() {
        ...
        if (hostSharedFormulaId != null) this.sheet().clearCellsUsingSharedFormula(hostSharedFormulaId);
    ...
    async toXml() {
        ...
        if (this._formulaType != null) {
            ...
            if (typeof this._value === "string") {
                type = "s";
                text = await this.workbook().sharedStrings().getIndexForString(this._value);
            } else if (typeof this._value === "boolean") {
                type = "b";
                text = this._value ? 1 : 0;
            } else if (typeof this._value === "number") {
                ...
                type = "s";
                text = await this.workbook().sharedStrings().getIndexForString(this._value.toXml());
            }
        ...
        if (this._style != null) {
            node.attributes.s = await this._style.id();
        } else if (this._styleId != null) {
            // style is inherited from row/column
            node.attributes.s = this.workbook().styleSheet().getRowColNewStyleId(this._styleId);
        }
    ...
    _init(nodeOrColumnNumber, styleId) {
        if (typeof nodeOrColumnNumber === "object") {
    ...
    _parseNode(node) {
        ...
        if (type === "s") {
            ...
            // Number value.
            const vNode = xmlq.findChild(node, 'v');
            this._value = (!vNode || vNode.children.length === 0) ? undefined : Number(vNode.children[0]);
        }
