...
class Sheet {
     ...
     */
    name() {
        return new ArgHandler('Sheet.name', arguments)
            .case(() => {
        ...
        return new ArgHandler('Sheet.dataValidation', arguments)
            .case('string', address => {
                ...
                if (node) {
                    return {
                        ...
                        allowBlank: node.attributes.allowBlank,
                        showInputMessage: node.attributes.showInputMessage,
                        prompt: node.attributes.prompt,
                        promptTitle: node.attributes.promptTitle,
                        showErrorMessage: node.attributes.showErrorMessage,
                        error: node.attributes.error,
                        errorTitle: node.attributes.errorTitle,
                        operator: node.attributes.operator,
                        formula1: node.children.length > 0 ? node.children[0].children[0] : undefined,
                        formula2: node.children.length > 1 ? node.children[1].children[0] : undefined
                    };
                } else {
                    return false;
                }
            ...
            .case(['string', '*'], (address, obj) => {
                if (typeof obj === 'string') {
                    this._dataValidations.set(address, {
                        name: 'dataValidation',
                        attributes: {
                            type: 'list',
                            allowBlank: false,
    ...
    toXmls() {
        ...
        // Add the hyperlinks if needed.
        const dataValidationsNode = this._dataValidations.toXml();
        if (dataValidationsNode.children.length) {
            xmlq.insertInOrder(node, dataValidationsNode, nodeOrder);
        }
    ...
    _init(workbook, idNode, node, relationshipsNode) {
        ...
        let dataValidationsNode = xmlq.findChild(this._node, "dataValidations");
        if (dataValidationsNode) {
            xmlq.removeChild(this._node, dataValidationsNode);
        } else {
            dataValidationsNode = { name: 'dataValidations', attributes: {}, children: [] };
        }
        this._dataValidations = new DataValidations(this, dataValidationsNode);


        // Create the hyperlinks.
        let hyperlinksNode = xmlq.findChild(this._node, "hyperlinks");
        if (hyperlinksNode) {
            xmlq.removeChild(this._node, hyperlinksNode);
        } else {
            hyperlinksNode = { name: 'hyperlinks', attributes: {}, children: [] };
        }
