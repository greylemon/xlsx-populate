...
class Sheet {
    ...
    deleteRow(rowNumber) {
        ...
        deletedRow._cells.forEach(cell => {
            ...
            const changes = this._workbook._refTable.getCalculationOrder(ref);
            changes.forEach(change => {
                totalChanges.push(change);
                const cell = this._workbook.sheet(change.sheet).row(change.row).cell(change.col);
        ...
        for (const i of keys) {
            if (i > rowNumber) {
                ...
                row._cells.forEach(cell => {
                    const changes = this._workbook._refTable.getCalculationOrder(cell.getRef());
                    changes.forEach(change => {
                        // if (change.sheet === this.name() && change.row > rowNumber) {
                        //     change.row--;
                        // }
                        ...
                        if (idxExist === -1) totalChanges.push(change);
                        const cell = this._workbook.sheet(change.sheet).row(change.row).cell(change.col);
                        const newFormula = FormulaReplacer.replaceRowNumber(cell.getFormula(), change.sheet,
                            this.name(), i, i - 1);
                        cell.setFormula(newFormula, false);
