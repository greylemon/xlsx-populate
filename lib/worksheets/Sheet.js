...
class Sheet {
    ...
    activeCell() {
        ...
        return new ArgHandler('Sheet.activeCell', arguments)
            .case(() => {
                ...
                const ref = addressConverter.fromAddress(cellAddress);
                if (ref.type === 'range') {
            ...
            .case('*', cellOrRange => {
                ...
                if (typeof cellOrRange === 'string') {
                    const ref = addressConverter.fromAddress(cellOrRange);
                    if (ref.type === 'range') {
                        cellOrRange = this.range(ref.startRowNumber, ref.startColumnNumber, ref.endRowNumber, ref.endColumnNumber);
                    } else {
                        cellOrRange = this.row(ref.rowNumber).cell(ref.columnNumber);
                    }
                }

                selectionNode.attributes.activeCell = selectionNode.attributes.sqref = cellOrRange.address();
                return this;
            })
            .handle();
    }
        ...
        for (const i of keys) {
            if (i > rowNumber) {
                ...
                row._cells.forEach(cell => {
                    const changes = this._workbook._refTable.getCalculationOrder(cell.getRef());
    ...
    deleteRow(rowNumber) {
        ...
        const keys = Array.from(this._rows.keys())
            .filter(element => element > rowNumber)
            .sort((n1, n2) => n1 - n2);
        for (const i of keys) {
            if (i > rowNumber) {
                const row = this._rows.get(i);
