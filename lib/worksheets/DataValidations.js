...
class DataValidations {
     ...
     *          type, error, operator, errorStyle, errorTitle, promptTitle, prompt}} - The data validation object.
     */
    getDataValidation(address) {
        ...
        try {
            ...
            dataValidation.formula1Result = this.parseFormula(dataValidation.formula1);
            dataValidation.formula2 = String(isX14 ? node.children[1].children[0].children[0] : node.children[1].children[0]);
            dataValidation.formula2Result = this.parseFormula(dataValidation.formula2);
    ...
    validate(cell, input, isFormula) {
        ...
        if (dataValidation.type === 'whole') { // whole number
            return typeof input === "number" && Math.trunc(input) === input
                && this._testNumber(input, dataValidation);
        } else if (dataValidation.type === 'decimal') {
            return typeof input === "number" && this._testNumber(input, dataValidation);
        } else if (dataValidation.type === 'list') {
            return dataValidation.formula1Result.includes(input);
        } else if (dataValidation.type === 'date') {
            return typeof input === "number" && this._testNumber(Math.trunc(input), dataValidation);
        } else if (dataValidation.type === 'time') {
            return typeof input === "number" && this._testNumber(input % 1, dataValidation);
        } else if (dataValidation.type === 'textLength') {
            return this._testNumber(String(input).length, dataValidation);
        } else if (dataValidation.type === 'custom') {
            return Boolean(dataValidation.formula1Result);
        }
    }
