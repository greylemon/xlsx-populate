...
class Style {
    ...
    getStyle(name) {
        if (!this._getter[name]) throw new Error(`_Style.style: '${name}' is not a valid style`);
        return this._getter[name]();
    }

    setStyle(name, value) {
        if (!this._setter[name]) throw new Error(`_Style.style: '${name}' is not a valid style`);
