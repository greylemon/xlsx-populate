...
class Style {
    ...
    getStyle(name) {
        if (!this._getter.has(name)) throw new Error(`_Style.style: '${name}' is not a valid style`);
        return this._getter.get(name)();
    }

    setStyle(name, value) {
        if (!this._setter.has(name)) throw new Error(`_Style.style: '${name}' is not a valid style`);
        this._setter.get(name)(value);
        return this;
    }
...
// using map to optimize object query time.
const getter = this._getter = new Map();
const setter = this._setter = new Map();
const entries = Object.entries(Style.prototype);
for (const [key, value] of entries) {
    const get = key.match(/_get_(.*)/);
    if (get) {
        getter.set(get[1], value);
        continue;
    }
    const set = key.match(/_set_(.*)/);
    if (set) {
        setter.set(set[1], value);
    }
}
