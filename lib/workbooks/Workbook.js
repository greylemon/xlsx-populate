...
class Workbook {
    ...
    /**
     * Return sheet Id by sheet name defined in workbook.xml
    ...
    getSheetIdByName(sheetName) {
        ...
        for (let i = 0; i < this._sheetsNode.children.length; i++) {
            ...
            if (child.attributes.name === sheetName)
                return child.attributes.sheetId;
        }
    }
    ...
    scopedDefinedName(sheetScope, name, refersTo) {
        ...
        let sheetId;
        if (sheetScope)
            sheetId = this.getSheetIdByName(sheetScope.name());
        let definedNameNode;
    ...
    _initAsync(data, opts) {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                // finish up dep parsing
                this._parser.unfinishedStack.forEach(cell => {
                    const dep = this._parser.parseDep(cell);
                    dep.forEach(refA => {
                        this._refTable.add(refA, cell.getRef());
                    });
                });
                this._parser.unfinishedStack = [];
            })
            .then(() => this);
    }
