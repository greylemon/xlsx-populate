...
class StyleSheet {
    ...
    optimize() {
        ...
        for (let i = 0, len = this._cellXfsNode.children.length; i < len; i++) {
            const xf = this._cellXfsNode.children[i];
            updateBorderId(xf);
            updateFillId(xf);
            updateFontId(xf);
        }
        for (let i = 0, len = this._cellStyleXfsNode.children.length; i < len; i++) {
            const xf = this._cellStyleXfsNode.children[i];
            updateBorderId(xf);
            updateFillId(xf);
            updateFontId(xf);
        }
