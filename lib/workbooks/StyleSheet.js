 ...
 * @returns {any[]}
 */
const optimizeNode = async (OuterNodeName, oldOuterNode, numOfReservedNodes, attributeName) => {
    const map = new Map();
    const newOuterNode = {
        name: OuterNodeName,
    ...
    for (let i = 0; i < numOfReservedNodes; i++) {
        ...
        map.set(await hash(oldInnerNode), i);
    }

    const updateXfNode = async xf => {
        const oldInnerNode = oldOuterNode.children[xf.attributes[attributeName]];
        const key = await hash(oldInnerNode);
        let idx = map.get(key);
        if (idx === undefined) {
...
class StyleSheet {
    ...
    createStyle(sourceId) {
        ...
        if (sourceId >= 0) {
            const sourceXfNode = this._cellXfsNode.children
                [this._oldStyleId2NewStyleId.size === 0 ? sourceId : this._oldStyleId2NewStyleId.get(sourceId)];
     ...
     */
    async getStyleId(xfNode) {
        return this._styleMap.get(await hash(xfNode));
    }
    ...
    async optimize() {
        ...
        const [newBordersNode, updateBorderId] = await optimizeNode('borders', this._bordersNode, 1, 'borderId');
        const [newFillsNode, updateFillId] = await optimizeNode('fills', this._fillsNode, 2, 'fillId');
        const [newFontsNode, updateFontId] = await optimizeNode('fonts', this._fontsNode, 1, 'fontId');

        for (const xf of this._cellXfsNode.children) {
            await updateBorderId(xf);
            await updateFillId(xf);
            await updateFontId(xf);
        }
