...
class StyleSheet {
    ...
    toXml() {
        ...
        const borders = {
            attributes: {},
            children: [
                // the first node is always no border. (default)
                this._bordersNode[0],
            ]
        };

        for (const xf of this._cellXfsNode) {
            const oldBorderId = xf.attributes.borderId;
            const oldNumFmtId = xf.attributes.numFmtId;
            const oldFillId = xf.attributes.fillId;
            const oldFontId = xf.attributes.fontId;
        }
        return this._node;
    }
