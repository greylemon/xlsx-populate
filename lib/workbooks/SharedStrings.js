...
class SharedStrings {
     ...
     * @param {string|Array.<{}>} stringOrRichText - The string or rich text array.
     * @returns {number} The index
     */
    getIndexForString(stringOrRichText) {
        // string that length smaller than 40 will not hash.
        const key = typeof stringOrRichText === 'string' && stringOrRichText <= HASH_LENGTH
            ? stringOrRichText : hash(stringOrRichText);
        ...
        this._stringArray.push(stringOrRichText);
    ...
    _cacheExistingSharedStrings() {
        this._node.children.forEach((node, i) => {
            ...
            if (content.name === "t") {
                ...
                this._indexMap[string.length <= HASH_LENGTH ? string : hash(string)] = i;
            } else {
                this._stringArray.push(node.children);
                this._indexMap[hash(node.children)] = i;
            }
        });
    }
